...
inline void assembling_all_descriptor( const float* H, size_t const *dims, const float* params, const float* grid, float* desc_out );
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  memset( desc_out, 0, sizeof(float)*DS*h*w );
  
  assembling_all_descriptor(H, dims1, params, grid, desc_out);
}
inline void assembling_all_descriptor( const float* H, size_t const *dims, const float* params, const float* grid, float* desc_out )
{
	int h            = dims[0];
	...
	for(int j = 0; j < w; j++)
	{
		for(int i = 0; i < h; i++)
		{
			...
			assembling_all_descriptor(H, dims1, params, grid, i, j, _desc_out);
   ...
   for(int nch=1;nch<=nChannl;nch++)
   {
	   ...
	   {		   
		   ...
		   else
		   {
			   yy = (int)fyy;
			   xx = (int)fxx;
			   float a = (float)yy - fyy;
			   float b = (float)xx - fxx;
			   //
			   int ny1 = yy;
			   int nx1 = xx+1;
			   int ny2 = yy+1;
